...
#import "ShoppingCartListCell.h"

@interface ShoppingCartListCell()
...
@implementation ShoppingCartListCell
...
- (void)setCellData:(ShoppingCartListModel *)cellData{
    _iconImage.image = [UIImage imageWithContentsOfFile:cellData.product_iconimage];
    _titleLabel.text = cellData.product_name;
    _priceLabel.text = [NSString stringWithFormat:@"￥%@",cellData.product_price];
    _goodsNumLabel.text = [NSString stringWithFormat:@"%@",cellData.number];
    _isSelectBtn.selected = cellData.isSelectButton;    //购物车内的商品选中状态等于传递过来的值
...
- (UIButton *)isSelectBtn{
    if (!_isSelectBtn ) {
        ...
        [_isSelectBtn setImage:[UIImage imageNamed:@"购物车界面商品选中对号按钮"] forState:(UIControlStateNormal)];
    }
    return _isSelectBtn;
}
